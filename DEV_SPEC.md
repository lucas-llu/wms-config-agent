...
**目标：** 构建统一、可配置且可观测的数据摄取流水线，覆盖 WMS/JDA MOCA 文档加载、格式解析、语义切分、领域元数据增强、多模态增强、嵌入计算、去重与批量上载到向量存储。该能力应是可重用的库模块，便于在 `ingest.py`、Dashboard 管理面板、离线批处理和测试中调用。

领域元数据至少包括：`source_path`、`document_title`、`section_path`、`page_or_line`、`product_version`、`module`、`site`、`environment`、`config_type`、`language`。无法从文档可靠提取的字段保留为空，不允许由 LLM 猜测补全。
...
- **备选：FastAPI + 自定义协议层**
	- **场景**：需要深度定制 HTTP 行为（如自定义中间件、复杂鉴权流程）时可考虑。
...
- **运维成本低**：结构化日志可直接用 `jq`、`grep` 等命令行工具查询；Dashboard 保持轻量，便于维护与扩展。
...
- `VALIDATING` 阶段仅允许证据缺口触发最多 2 轮定向重检索；版本/模块/站点/环境冲突、DAG 错误或重试耗尽必须进入可恢复暂停，任何 blocking conflict/finding 都不得进入 `REVIEW_REQUIRED`；
...
- **实现要点**：
  - `app.py` 使用 `st.navigation()` 注册七个页面；V1 六页与 V2 Agent Sessions 页均为完整实现
